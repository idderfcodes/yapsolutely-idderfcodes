...
export default function AnimatedTestimonials() {
  ...
  const row2 = [...testimonials.slice(4, 8), ...testimonials.slice(4, 8), ...testimonials.slice(4, 8)];

  return (
    <section ref={containerRef} className="py-16 sm:py-24 overflow-hidden bg-[var(--color-bg)] relative">
      <div className="landing-container mb-12 flex flex-col items-center justify-center text-center">
        ...
        <h2 className="landing-display landing-display-2 text-[var(--color-text-primary)]">
          Don't just take our word for it
        </h2>
        <p className="relative z-10 landing-body landing-body-1-regular mt-4 max-w-[32rem] text-[var(--color-text-muted)]">
      ...
      <div className="relative flex flex-col gap-6">
        ...
        >
          {row1.map((item, idx) => (
            <div key={`r1-${idx}`} className="group relative w-[320px] sm:w-[380px] shrink-0 rounded-[24px] bg-[var(--color-bg-secondary)] p-6 transition-all duration-300 hover:-translate-y-1">
              ...
              <div className="relative z-10 flex items-center gap-3">
                <img src={item.avatar} alt={item.author} className="w-11 h-11 rounded-full object-cover border border-[var(--color-border)]" />
                <div>
        ...
        >
          {row2.map((item, idx) => (
            <div key={`r2-${idx}`} className="group relative w-[320px] sm:w-[380px] shrink-0 rounded-[24px] bg-[var(--color-bg-secondary)] p-6 transition-all duration-300 hover:-translate-y-1">
              {/* Animated gradient border on hover */}
              <div className="absolute inset-0 rounded-[24px] bg-gradient-to-br from-[var(--color-accent-primary)] to-[var(--color-accent-secondary)] opacity-0 blur-[2px] transition-opacity duration-300 group-hover:opacity-[0.15]" />
              <div className="absolute inset-0 rounded-[24px] border border-[var(--color-border)] transition-colors duration-300 group-hover:border-[var(--color-accent-primary)]/30" />
        ...
        <div className="pointer-events-none absolute inset-y-0 left-0 w-[15%] bg-gradient-to-r from-[var(--color-bg)] via-[var(--color-bg)]/80 to-transparent" />
        <div className="pointer-events-none absolute inset-y-0 right-0 w-[15%] bg-gradient-to-l from-[var(--color-bg)] via-[var(--color-bg)]/80 to-transparent" />
      </div>
    </section>
  );
}
