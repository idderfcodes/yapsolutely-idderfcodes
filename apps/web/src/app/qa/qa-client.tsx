  ...
  return (
    <DashboardLayout>
      <div className="p-5 sm:p-6 lg:p-8 max-w-[1100px]">
        {/* ── Header ── */}
        <div className="mb-5">
          <h1 className="font-display text-[1.38rem] font-semibold tracking-[-0.02em] text-text-strong">Quality Assurance</h1>
          <p className="font-body text-[0.89rem] text-text-subtle mt-0.5">Review agent performance and flag issues.</p>
        </div>
        ...
        <div className="bg-surface-panel rounded-card border border-border-soft overflow-hidden mb-5">
          <div className="px-4 py-3 border-b border-border-soft/60 flex items-center justify-between">
            <h3 className="font-display text-[0.89rem] font-medium text-text-strong">Review Queue</h3>
            <div className="flex items-center gap-1.5">
              {["All", "Flagged", "Passed"].map((tab, i) => (
                <button
                  key={tab}
                  className={`font-body text-[0.8rem] px-2 py-0.5 rounded ${i === 0 ? "bg-foreground text-primary-foreground" : "text-text-subtle hover:text-text-body"}`}
                >
                  {tab}
                </button>
              ))}
        ...
        <div className="bg-surface-panel rounded-card border border-border-soft px-6 py-10 text-center">
          ...
          <h3 className="font-display text-[1.02rem] font-medium text-text-strong mb-0.5">No reviews yet</h3>
