    ...
    <DashboardLayout>
      <div className="p-5 sm:p-6 lg:p-8 max-w-[1100px]">
        ...
        <div className="flex items-center justify-between mb-5">
          <div>
            <h1 className="font-display text-[1.38rem] font-semibold tracking-[-0.02em] text-text-strong">Analytics</h1>
            <p className="font-body text-[0.89rem] text-text-subtle mt-0.5">Usage metrics and performance trends.</p>
          </div>
          <div className="flex items-center gap-1.5">
            {["7d", "30d", "90d"].map((range) => (
              <button
                key={range}
                className={`font-body text-[0.8rem] px-2 py-0.5 rounded ${range === "30d" ? "bg-foreground text-primary-foreground" : "text-text-subtle hover:text-text-body"}`}
              >
                {range}
              </button>
            ))}
          ...
          {["Call Volume", "Duration Distribution"].map((title) => (
            <div key={title} className="bg-surface-panel rounded-card border border-border-soft overflow-hidden">
              <div className="px-4 py-3 border-b border-border-soft/60">
                <h3 className="font-display text-[0.89rem] font-medium text-text-strong">{title}</h3>
              </div>
              <div className="px-4 py-12 flex flex-col items-center justify-center">
                <BarChart3 className="w-5 h-5 text-text-subtle/30 mb-2" />
                <p className="font-body text-[0.89rem] text-text-subtle/60">Chart data appears after calls</p>
              </div>
            </div>
          ))}
        </div>
        ...
        <div className="bg-surface-panel rounded-card border border-border-soft overflow-hidden">
          <div className="px-4 py-3 border-b border-border-soft/60">
            <h3 className="font-display text-[0.89rem] font-medium text-text-strong">Agent Performance</h3>
          </div>
