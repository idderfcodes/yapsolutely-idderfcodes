    ...
    <div className="min-h-screen bg-canvas flex">
        ...
        <div className="flex-1 flex items-center justify-center">
          <div className="w-full max-w-content-auth animate-fade-in" style={{ animationDelay: "0.05s" }}>
            ...
            <p className="font-body text-body-md text-text-subtle leading-relaxed mb-8">
              Set up your workspace and start building voice agents.
            </p>
      ...
      <div className="hidden lg:flex flex-1 bg-surface-dark rounded-l-[2rem] p-12 xl:p-16 items-center justify-center relative overflow-hidden">
        <div className="max-w-md animate-fade-in" style={{ animationDelay: "0.2s" }}>
          <span className="font-body text-[0.6rem] text-surface-dark-foreground/20 uppercase tracking-[0.2em] block mb-6">Getting started</span>
          ...
          <div className="space-y-4">
            {[
              { icon: Bot, step: "01", label: "Create your first agent" },
              { icon: Headphones, step: "02", label: "Assign a phone number" },
              { icon: FileText, step: "03", label: "Review calls and transcripts" },
            ].map(({ icon: Icon, step, label }) => (
              <div key={step} className="flex items-center gap-4 px-4 py-3 rounded-xl bg-surface-dark-foreground/5">
                <span className="font-mono text-[0.6rem] text-surface-dark-foreground/15">{step}</span>
                <Icon className="w-4 h-4 text-surface-dark-foreground/25" />
                <span className="font-body text-[0.78rem] text-surface-dark-foreground/45">{label}</span>
              </div>
            ))}
          </div>
        </div>
